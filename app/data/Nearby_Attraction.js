export const Nearby_Attraction_Data = [
  {
    id: 1,
    Name: "Gorumara National Park",
    Description:
      "A wildlife sanctuary in West Bengal, home to Indian rhinoceroses, lush greenery, diverse fauna, and serene landscapes.",
    Image:
      "https://firebasestorage.googleapis.com/v0/b/coffee-shop-b6e1a.appspot.com/o/Green_Vista_Resort%2Fgorumara-popularity%201.jpg?alt=media&token=b1f94dd2-1dc0-4a33-a317-026b297a07c0",
  },
  {
    id: 2,
    Name: "Coochbehar",
    Description:
      "A historic town famous for Cooch Behar Palace, temples, rich cultural heritage, scenic beauty, and vibrant local traditions.",
    Image:
      "https://firebasestorage.googleapis.com/v0/b/coffee-shop-b6e1a.appspot.com/o/Green_Vista_Resort%2FCoochbehar.jpg?alt=media&token=42dab326-34ec-4fed-8e01-09e1c81712c2",
  },
  {
    id: 3,
    Name: "Malbazar",
    Description:
      "A scenic town in Dooars with picturesque tea gardens, beautiful landscapes, peaceful ambiance, and access to national parks.",
    Image:
      "https://firebasestorage.googleapis.com/v0/b/coffee-shop-b6e1a.appspot.com/o/Green_Vista_Resort%2FMalbazzar.jpg?alt=media&token=6a3c145a-deaa-4202-abb4-c2571f96430b",
  },
  {
    id: 4,
    Name: "Darjeeling",
    Description:
      "A world-famous hill station known for tea plantations, stunning Himalayan views, cultural richness, and Darjeeling Himalayan Railway.",
    Image:
      "https://firebasestorage.googleapis.com/v0/b/coffee-shop-b6e1a.appspot.com/o/Green_Vista_Resort%2FDarjeeling.jpg?alt=media&token=da1e5ca0-5bc3-488b-b16d-06a647cf4c62",
  },
  {
    id: 5,
    Name: "Sikkim",
    Description:
      "A northeastern paradise with breathtaking monasteries, snow-capped mountains, diverse wildlife, vibrant culture, and spectacular natural beauty.",
    Image:
      "https://firebasestorage.googleapis.com/v0/b/coffee-shop-b6e1a.appspot.com/o/Green_Vista_Resort%2FSikkim.jpg?alt=media&token=3e8e3698-aa4e-40db-8a46-b1fe751d0865",
  },
  {
    id: 6,
    Name: "Raimatang",
    Description:
      "A hidden gem in Dooars with lush forests, tranquil rivers, scenic charm, and peaceful retreats amidst nature's beauty.",
    Image:
      "https://firebasestorage.googleapis.com/v0/b/coffee-shop-b6e1a.appspot.com/o/Green_Vista_Resort%2FRaimatang.jpg?alt=media&token=782fb487-5e20-4fb5-aace-6ff9b4f10199",
  },
];
