...
const MediaItem = ({ item, className, onClick }) => {
  ...
  // Ensure mediaUrl exists before rendering
  if (!item.mediaUrl || item.mediaUrl === "") {
    return <div className="w-full h-full bg-gray-200">No media available</div>;
  }
  ...
  return (
    <Image
      src={item.mediaUrl}
      alt={item.title}
      className={`${className} object-fill cursor-pointer`}
      onClick={onClick}
      loading="lazy"
      width={2000}
      height={2000}
...
// GalleryModal component (unchanged except for prop consistency)
const GalleryModal = ({
  selectedItem,
  isOpen,
  onClose,
          ...
          <div className="flex items-center -space-x-2 px-3 py-2">
            {mediaItems.map((item, index) => (
              <motion.div
                ...
                className={`
                  relative group w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 flex-shrink-0 rounded-lg overflow-hidden cursor-pointer hover:z-20
                  ${
                    selectedItem.id === item.id
                ...
                <motion.div
                  ...
                  <motion.div
                    ...
                    <p className="relative text-white/70 text-[10px] sm:text-xs md:text-sm mt-0.5 line-clamp-2">
                      {item.description} {/* Changed desc to description */}
                    </p>
                  </motion.div>
                </motion.div>
              ))}
